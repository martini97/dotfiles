...
local function ruby_provider()
  local ruby_provider = vim.fn.exepath("neovim-ruby-host")
  if file.exists(ruby_provider) then return ruby_provider end

  ruby_provider = vim.fn.expand("~/.gem/ruby/*/bin/neovim-ruby-host")
  if file.exists(ruby_provider) then return ruby_provider end
end
