...
function utils.isempty(s) return s == nil or s == "" end

function utils.safe_get_option(ns, name)
    local success, value = pcall(function() return vim[ns][name] end)
...
-- TODO(martini97, 2021-04-02): add option to only search in the current file
-- filetype.
...
function utils.goto_tag()
    if not utils.tags_exists() then log.error("Tags file not found.") end
